...
export async function POST(request: Request) {
  try {
    ...
    // Execute hit in a transaction to prevent race conditions
    let updatedGame;
    try {
      updatedGame = await db.transaction(async (tx) => {
        ...
        if (dbGame.status !== "playing") {
          throw new Error("Cannot hit - game is not in playing status");
        }
      ...
      console.error("Transaction error during hit:", error);
      if (error instanceof Error) {
        ...
        if (error.message === "Cannot hit - game is not in playing status") {
          return NextResponse.json(
            { error: error.message },
            { status: 400 },
          );
