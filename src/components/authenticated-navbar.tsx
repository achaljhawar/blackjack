...
export function AuthenticatedNavbar({
  ...
  return (
    <header className="border-border bg-card sticky top-0 z-50 flex items-center justify-between border-b p-4 transition-colors duration-300 md:px-6">
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        {chips !== undefined && (
          <div className="bg-primary/10 flex items-center gap-2 rounded-full px-3 py-1.5 md:px-4 md:py-2">
            <Coins className="h-4 w-4 text-yellow-500 md:h-5 md:w-5" />
            ...
            {onBuyChips && (
              <Button
                variant="ghost"
                size="icon"
                className="bg-primary/20 hover:bg-primary/30 h-5 w-5 rounded-full md:h-6 md:w-6"
                onClick={onBuyChips}
              >
      ...
      <div className="flex items-center gap-2 md:hidden">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              className="hover:bg-accent relative h-9 w-9 rounded-full p-0"
            >
              <Avatar className="border-border h-9 w-9 border-2">
                <AvatarImage
                  src={session.user?.image ?? undefined}
                  alt={getUserDisplayName()}
                />
                <AvatarFallback className="bg-gradient-to-br from-yellow-500 to-orange-600 text-xs font-bold text-white">
                  {getUserInitials()}
                </AvatarFallback>
              </Avatar>
            </Button>
          ...
          >
            ...
            <nav className="mt-8 flex flex-col gap-4">
              ...
              <button
                ...
                className="text-foreground hover:bg-accent flex items-center gap-3 rounded-lg px-3 py-3 text-left text-base font-medium transition-colors"
              >
                <Sun className="h-5 w-5 scale-100 rotate-0 transition-all dark:scale-0 dark:-rotate-90" />
                <Moon className="absolute ml-3 h-5 w-5 scale-0 rotate-90 transition-all dark:scale-100 dark:rotate-0" />
